...
def test_python_ignores_a_range(tmp_path: Path) -> None:
    ...
    assert (
        versions(directory, tmp_path, "python", "x", "flask").instrumented
        is None
    )
...
def test_dotnet_stops_at_the_root(tmp_path: Path) -> None:
    ...
    directory.mkdir(parents=True)
    assert (
        versions(directory, tmp_path, "dotnet", "X", "y").instrumentation
        is None
    )
...
def test_java_does_not_guess_when_the_label_is_not_the_artifact(
    ...
    assert (
        versions(leaf, tmp_path, "java", "x:y", "servlet").instrumented is None
    )
