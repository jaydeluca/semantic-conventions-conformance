...
import target from './views/target.js';
import signals from './views/signals.js';
import attributes from './views/attributes.js';
import findings from './views/findings.js';

const ROUTES = [
  { name: 'overview', match: /^\/?$/, view: overview },
  { name: 'overview', match: /^\/target\/(.+)$/, view: target },
  { name: 'signals', match: /^\/signals(?:\/(.+))?$/, view: signals },
  { name: 'attributes', match: /^\/attributes$/, view: attributes },
  { name: 'findings', match: /^\/findings(?:\/(.+))?$/, view: findings },
];
...
function render(data) {
  ...
  for (const anchor of document.querySelectorAll('.masthead nav a')) {
    if (anchor.dataset.route === route.name) {
  ...
  document.title = `${route.name === 'overview' && argument ? argument : route.name} · conformance`;
}
